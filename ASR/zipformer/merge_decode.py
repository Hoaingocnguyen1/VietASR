...
from ASR.zipformer.model.beam_search import (
    ...
    greedy_search,
    merge_greedy_search_batch,
    merge_modified_beam_search,
...
def get_parser():
    ...
    parser.add_argument(
        ...
        type=str,
        default="all",
        help="The cuts name. It is used to load the corresponding dataset",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="The cuts path, used when cuts_name is x",
    )
...
def decode_one_batch(
    ...
    Returns:
      ...
      the returned dict.
    """
    device = next(model_lis[0].parameters()).device
    ...
    encoder_out_lis = []
    encoder_out_lens_lis = []

    for model in model_lis:
        encoder_out, encoder_out_lens = model.forward_encoder(feature, feature_lens)
        encoder_out_lis.append(encoder_out)
        encoder_out_lens_lis.append(encoder_out_lens)

    hyps = []

    if params.decoding_method == "greedy_search":
        hyp_tokens = merge_greedy_search_batch(
            model_lis=model_lis,
            encoder_out_lis=encoder_out_lis,
            encoder_out_lens_lis=encoder_out_lens_lis,
        )
        for hyp in sp.decode(hyp_tokens):
            hyps.append(hyp.split())
    elif params.decoding_method == "modified_beam_search":
        hyp_tokens = merge_modified_beam_search(
            model_lis=model_lis,
            encoder_out_lis=encoder_out_lis,
            encoder_out_lens_lis=encoder_out_lens_lis,
...
def main():
    ...
    args.epoch_lis = str_to_list(args.epoch, target_n)
    args.iter_lis = str_to_list(args.iter, target_n)
    args.avg_lis = str_to_list(args.avg, target_n)
    ...
    params.suffix = f"epoch-{params.epoch.replace(',', '_')}-iter-{params.iter.replace(',', '_')}-avg-{params.avg.replace(',', '_')}"
    ...
    for i in range(target_n):
        ...
        if not params.use_averaged_model:
            if params.iter_lis[i] > 0:
                ...
                if len(filenames) == 0:
                    raise ValueError(
                        ...
                        f" --iter {params.iter_lis[i]}, --avg {params.avg_lis[i]}"
                    )
                elif len(filenames) < params.avg_lis[i]:
                    raise ValueError(
                ...
                filenames = []
                for i in range(start, params.epoch_lis[i] + 1):
                    if i >= 1:
                        filenames.append(f"{params.checkpoint_dir[i]}/epoch-{i}.pt")
                logging.info(f"averaging {filenames}")
                model.to(device)
                model.load_state_dict(average_checkpoints(filenames, device=device))
        else:
            if params.iter_lis[i] > 0:
                ...
                elif len(filenames) < params.avg_lis[i] + 1:
                    raise ValueError(
                        f"Not enough checkpoints ({len(filenames)}) found for"
                        f" --iter {params.iter_lis[i]}, --avg {params.avg_lis[i]}"
                    )
                ...
                assert params.avg_lis[i] > 0, params.avg_lis[i]
